<?php
	...
	if(checkEmail($dados['email']) and checkDomain($dados['email'])){
		...
		$send = @mail_utf8("vendas@noxi.com.br","Form Seja um cliente",$texto,"From: ".$dados['email']);
		if($send){
	...
	if($erro){
		...
		echo "<script>window.location.href='clientes.html'</script>";
	}
	
	// $redirect = $_SERVER['HTTP_REFERER'];
	// echo '<meta http-equiv="refresh" content="0;URL='.$redirect.'" />';
	echo "<script>alert('$sucesso');</script>";
	echo "<script>window.location.href='clientes-resposta.html'</script>";
?>
